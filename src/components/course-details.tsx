      ...
      <div className="mx-auto max-w-[90rem]">
        <div className="grid gap-8 border-b border-border pb-10 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] lg:items-end">
          <div>
            ...
            <h2 className="mt-4 max-w-[12ch] text-[clamp(2.4rem,4.5vw,4.8rem)] font-semibold leading-[0.96] tracking-[-0.06em] text-foreground">
              Kursus yang mengubah konsep kepada execution.
            </h2>
          </div>
          <div className="grid gap-5 sm:grid-cols-3">
            <div>
              <p className="font-mono text-[0.66rem] tracking-[0.09em] text-muted-foreground">TARIKH</p>
              <p className="mt-2 text-sm font-semibold leading-6 text-foreground">{courseOffer.date}</p>
            </div>
            <div>
              <p className="font-mono text-[0.66rem] tracking-[0.09em] text-muted-foreground">MASA</p>
              <p className="mt-2 text-sm font-semibold leading-6 text-foreground">{courseOffer.schedule.join(' · ')}</p>
            </div>
            <div>
              <p className="font-mono text-[0.66rem] tracking-[0.09em] text-muted-foreground">PLATFORM</p>
              <p className="mt-2 text-sm font-semibold leading-6 text-foreground">{courseOffer.platform}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col justify-between gap-5 border-t border-border pt-8 sm:flex-row sm:items-end">
          <p className="font-mono text-xs tracking-[0.12em] text-muted-foreground">EDUCATOR · {courseOffer.educator.toUpperCase()} · CERTIFIED TECHNICAL ANALYST</p>
          <p className="font-[family-name:var(--font-display)] text-4xl font-semibold tracking-[-0.06em] text-foreground">
            <span className="mr-3 text-lg font-medium text-muted-foreground line-through">{courseOffer.previousPrice}</span>
            {courseOffer.price}
          </p>
        </div>
      </div>
